...
>A useful heuristic to determine whether an expression is an lvalue is to ask if you can  take its address. If you can, it typically is. If you can’t, it’s usually an rvalue. - *Effective Modern C++ by Scott Meyers*
    
右值引用（&&）的作用：区分“即将被销毁的对象”，从而安全、高效地“移动”资源，而不是拷贝。

但是我不理解为什么 C++ Primer 第十三章明明在讲 Copy Control ，却提到了 Rvalue References 的概念？这和 Copy Control、以及紧接着讲的 move 有什么关系？

这里的脉络其实是：move 这一语义的提出相比 copy 提升了效率，而为了安全地 move 资源，C++ 才不得不引入右值引用。
...
if constexpr (std::is_nothrow_move_constructible<T>::value
    ...
    // 如果 move 是异常安全的，或者 copy 根本不可用，使用 move 构造
    uninitialized_move(...)
} else {
    // 使用 copy 构造
    uninitialized_copy(...)
...
之前我们在实验里为了强行使用 move, 使用 `-fno-elide-constructors` 编译选项。如果不开这个选项，甚至连 move 都不会发生。

C++17 提供了一种比 move 更彻底的机制: copy elision.

在 C++17 之后，返回值对象直接构造在调用者的存储位置。没有 copy，没有 move.

🚧 这部分并不在 C++ Primer 这本书的介绍范围内，TODO: 读 Effective Modern C++ RVO, copy elision 的部分。🚧 

---

参考阅读：

1. [一文读懂 C++ 右值引用和 std::move](https://zhuanlan.zhihu.com/p/335994370)
2. [text](https://fekir.info/post/guaranteed-copy-elision/)